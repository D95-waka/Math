...
\begin{example}
	...
	נבחן את הערך הממשי שלה, $x_n = \re(z_n) = 2^{1/n} \xrightarrow{x \to \infty} 1$.
	באופן דומה $y_n = \im(z_n) = 2^{-n} \xrightarrow{x \to \infty} 0$.
	ולכן $z_n \to 1$.
\end{example}
\begin{example}
	אם לעומת זאת $z_n = {(-1)}^n 2^{1/n} + i 2^{-n}$,
...
\begin{definition}[קשירות]
	...
	\begin{enumerate}
		...
		\item קשירות פוליגוניאלית: לכל $z, w \in G$ קיים $z \le a_1, \dots, a_n \le w$ כך ש־$G \supseteq [a_j, a_{j + 1}]$. \\*
			נבחין כי $[z, w] = \{ t \cdot z + (1 - t) \cdot w \mid t \in [0, 1]\}$. \\*
			הרעיון הוא שיש קבוצת כדורים פנימיים בקבוצה שמחברים בין שתי נקודות, מעין יצירת מסלול בין כל שתי נקודות על־ידי יצירת כדורים.
...
עתה $P = (x_0, y_0, z_0) \in S^2$, דהינו $x_0^2 + y_0^2 + z_0^2 = 1$, אך עדיין אם $\phi^{-1}(P) = z$ אז $P \in L_z$ ובהתאם $z \in L_p$, כאשר
\[
...
אם כך נקבל
\[
	t z + (1 - t) = 0 \iff t(z - 1) = -1 \implies t = \frac{1}{1 - z_0}
\]
אז
\[
	\re(z) = \frac{x_0}{1 - z_0},
